...
namespace ProjectBankApp.Pages.Transactions
{
    ...
    public class SendMoneyModel : PageModel
    {
        ...
        [BindProperty]
        [Display(Name = "ReceiverIFSC")]
        [Required]
        public string ReceiverIFSC { get; set; }


        [BindProperty]
        [Display(Name = "ReciverAccNo")]
        [Required]
        ...
        public void OnPost()
        {
            ...
            var newTransaction = new TransactionsDataModel
            {
                ...
                Remarks = $"Rs.{Amount}/- has been Debited from your Account No. XXX{accNo.Substring(accNo.Length-4)} using Send Money Option. Money sent to Reciever Name: {ReceiverName}, Acc No: {ReceiverAccNo}, IFSC Code: {ReceiverIFSC} on {DOT}. Your Current Balance is {closingBalance}"
            ...
            if (insertedTransaction != null || insertedTransaction.Id > 0)
            {
                ...
                if (result)
                {
                    SuccessMessage = $"Transaction Successful! Rs.{Amount} has been Debited from your account as a mode of online fund transfer. Your current Balance is {closingBalance}";
                    return;
                }
            }
